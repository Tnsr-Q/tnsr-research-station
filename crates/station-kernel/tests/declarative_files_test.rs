...
fn default_profile_loads() {
    ...
    assert!(
        !profile.plugin_manifests.is_empty(),
        "Profile should reference plugin manifests"
    );
    assert!(
        !profile.schema_files.is_empty(),
        "Profile should reference schema files"
    );
}
...
fn quantum_plugin_manifest_loads() {
    ...
    let manifest =
        load_plugin_manifest_json(&plugin_path).expect("Failed to load quantum plugin manifest");

    assert_eq!(manifest.id, "adapter_quantum");
    assert_eq!(manifest.kind, PluginKind::Compute);
    assert!(
        !manifest.publishes.is_empty(),
        "Quantum plugin should publish topics"
    );
}
...
fn rag_plugin_manifest_loads() {
    ...
    let manifest =
        load_plugin_manifest_json(&plugin_path).expect("Failed to load RAG plugin manifest");

    assert_eq!(manifest.id, "adapter_rag");
    assert_eq!(manifest.kind, PluginKind::Semantic);
    assert!(
        !manifest.publishes.is_empty(),
        "RAG plugin should publish topics"
    );
}
...
fn quantum_state_schema_loads() {
    ...
    assert_eq!(
        ...
        3,
        "Schema should have 3 required fields"
    );
    assert!(!schema.schema_hash.is_empty(), "Schema should have a hash");
}

#[test]
fn plugin_capability_schema_loads_with_hardened_types() {
    let schema_path = repo_root().join("schemas/plugin-capability.schema.json");
    let schema = load_schema_json(&schema_path).expect("Failed to load plugin capability schema");

    assert_eq!(schema.id, "tnsr.plugin.capability.v1");
    assert_eq!(schema.topic, "plugin.capability");
    assert_eq!(
        schema.required_fields.len(),
        4,
        "Schema should have 4 required fields"
    );
    assert!(schema
        .required_fields
        .contains(&("enabled".to_string(), FieldType::Boolean)));
    assert!(schema
        .required_fields
        .contains(&("deterministic".to_string(), FieldType::Boolean)));
    assert!(schema
        .required_fields
        .contains(&("claims".to_string(), FieldType::Object)));
    assert!(schema
        .required_fields
        .contains(&("supported_topics".to_string(), FieldType::Array)));
}
...
fn quantum_schema_has_expected_fields() {
    ...
    let schema = registry
        .schema_for_topic("quantum.state")
...
fn proto_file_exists() {
    ...
    assert!(
        proto_path.exists(),
        "Protocol buffer definition should exist"
    );

    let content = std::fs::read_to_string(&proto_path).expect("Failed to read proto file");
    assert!(
        content.contains("EventEnvelope"),
        "Proto should define EventEnvelope"
    );
    assert!(
        content.contains("version"),
        "Proto should include version field"
    );
    assert!(
        content.contains("parent_id"),
        "Proto should include parent_id field"
    );
    assert!(
        content.contains("created_at_ms"),
        "Proto should include created_at_ms field"
    );
    assert!(
        content.contains("schema_hash"),
        "Proto should include schema_hash field"
    );
}
